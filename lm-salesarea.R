...
df = df3
mean(df3$X); mean(df3$Y)
sum(df3$X); sum(df3$Y)
cov(df3$X,df3$Y); cov(df3$Y,df3$X)
cor(df3$X,df3$Y) ; cor(df3$Y,df3$X)
cor.test(df3$X,df3$Y)
?plot
?cor.test

df1 = data.frame(df3$X, df3$Y)
df1 = data.frame(df3$X,df3$Y)
head(df1)

plot(y=df3$Y, x=df3$X,xlab='Area in sqft', ylab='Sales Amount', type='p', ylim = c(0,max(df3$Y)),
     main='Plot of Area Vs Sales', xlim = c(0,max(df3$X)),
     col='red',pch = 19)
abline(lm(df3$Y ~ df3$X), lty = 3, lwd = 4, col = 'green')
abline(v=c(3,5),h=c(6,10), col=c('red','blue'))
#Model
?lm
fit1 = lm(df3$Y ~ df3$X, data=df3)
fit1
summary(fit1)
names(fit1)
...
fitted(fit1)
cbind(df3,fitted(fit1),fitted(fit1)-df3$Y, residuals(fit1))
cbind(df3, fitted(fit1))
range(df3$X)
new1 = data.frame(X=c(1.5,2,3,4,5))
new1
new2 = data.frame(X = c(2,3))
predict(fit1, new2)
p1= predict(fit1, newdata= new1)

#df1$predict = predict(fit1, newdata= new2)
df3$predict = fitted(fit1)
head(df1)

#Variation
(SSE = sum((df3$Y - df3$predict)^2))
(SSR = sum((df3$predict - mean(df3$Y))^2))
#11.2 + 105.74
(SST = sum((df3$Y - mean(df3$Y))^2))
